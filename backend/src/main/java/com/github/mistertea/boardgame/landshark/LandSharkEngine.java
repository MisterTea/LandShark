...
public class LandSharkEngine extends DieRollEngine {
  ...
  protected boolean processInput(LandsharkCommand command) throws IOException {
    ...
    if (game.commands.isEmpty()) {
      ...
      int position = Collections.binarySearch(game.commands, command,
          new Comparator<LandsharkCommand>() {
            @Override
            public int compare(LandsharkCommand o1, LandsharkCommand o2) {
    ...
    case BUY_HOUSES:
      ...
      if (!query.canPutHouses(currentState, command.player, group)) {
        ...
        throw new IOException("Invalid house allocation: " + command.player
            + " " + group);
      } else {
        putHouses(group);
        finishEndOfTurn();
    ...
    case CREATE_AUCTION:
      ...
      Assert.assertEquals("Invalid auction choice: " + command.toString(),
          query.getOwner(currentState, board.properties.get(command.property)),
          command.player);
      currentState.turnState = TurnState.AUCTION;
      currentState.auctionState = new LandsharkAuctionState().setProperty(
          command.property).setAuctionOwner(command.player);
      snapshot(command.player + " started an auction for " + command.property);
      break;
    case PASS_TURN:
  ...
  private void putHouses(String group) {
    ...
    snapshot(query.getCurrentPlayerState(currentState).name
        + " bought houses on his " + group + " properties.");
  }
  ...
  private void completeAuction() {
      ...
      if (highestBidder == null) {
        ...
        if (highestBid == entry.getValue()) {
          ...
          while (true) {
            ...
            onPlayer = (onPlayer + 1) % currentState.playerStates.size();
          }
    ...
    if (winningBid <= 0) {
      ...
      currentState.stats.propertyOwnerStats.get(auctionState.property).add(
  ...
  private void beginNewTurn() {
    ...
    case TAXES:
      ...
      snapshot(currentPlayerState.name + " draws a taxes and loses "
          + DIVIDEND_PAYOUT/2 + ".  Everyone else loses " + (DIVIDEND_PAYOUT / 4)
          + ".");
      finishAction();
      break;
    default:
  ...
  private void finishAction() {
    ...
    // to take an action.
    if (query.canCurrentPlayerBuildHouses(currentState)
  ...
  private void updateGame() {
    ...
    JacksonDBCollection<LandsharkGame, String> gameCollection = MongoJackService
  ...
  public static void main(String[] args) throws IOException, TException {
    LandsharkBox newBoard = new LandsharkBox();
    ...
    LandsharkBox board = boxCollection.find().sort(DBSort.desc("timestamp"))
        .next();
    ...
    game = dbService.getCollection(LandsharkGame.class).insert(game)
        .getSavedObject();
    LandSharkEngine engine = new LandSharkEngine(new Random(1L), board, game);
    engine.initialize();
    ...
    while (!engine.isGameOver()) {
      ...
      for (AbstractPlayer playerController : playerControllers.values()) {
        ...
        LandsharkCommand command = playerController.fetchCommand(engine.query,
            engine.currentState);
