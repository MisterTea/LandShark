...
public class StateQuery {
	...
	public int getRent(LandsharkState currentState, Property property) {
		...
		if (currentState.groupHouseCount.containsKey(property.group)) {
			return getRentForStreet(board.propertyGroups.get(property.group),
					true, currentState.groupHouseCount.get(property.group));
		} else {
			return getRentForStreet(board.propertyGroups.get(property.group),
	...
	public int getRentForStreet(PropertyGroup group, boolean ownsGroup,
			int houses) {
		if (houses > 0) {
	...
	public PropertyGroup getGroupForProperty(String property) {
	...
	public boolean someoneCanAfford(LandsharkState currentState, int money) {
		for (PlayerState playerState : currentState.playerStates) {
			if (playerState.cash >= money) {
				return true;
			}
		}
		return false;
